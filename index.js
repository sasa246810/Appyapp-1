...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.customId === 'setup_channel_select') {
    ...
    if (interaction.customId === 'apply') {
      ...
      // ... التكملة من الكود اللي انتهيت عنده ...
...
const experienceComponent = new TextInputBuilder()
  .setCustomId('q3')
  .setLabel('خبراتك في الإدارة أو العمل التطوعي؟')
  .setStyle(TextInputStyle.Paragraph)
  .setRequired(true);

const motivationComponent = new TextInputBuilder()
  .setCustomId('q4')
  .setLabel('لماذا تريد الانضمام؟')
  .setStyle(TextInputStyle.Paragraph)
  .setRequired(true);

const availabilityComponent = new TextInputBuilder()
  .setCustomId('q5')
  .setLabel('ما هي أوقات تواجدك؟')
  .setStyle(TextInputStyle.Short)
  .setRequired(true);

modal.addComponents(
  new ActionRowBuilder().addComponents(nameComponent),
  new ActionRowBuilder().addComponents(ageComponent),
  new ActionRowBuilder().addComponents(experienceComponent),
  new ActionRowBuilder().addComponents(motivationComponent),
  new ActionRowBuilder().addComponents(availabilityComponent),
);
...
if (interaction.isModalSubmit()) {
  if (interaction.customId === 'staff_apply') {
    ...
    try {
      ...
      const existingApp = await Application.findOne({ userId: interaction.user.id });
      if (existingApp) {
        existingApp.applications.push({ q1, q2, q3, q4, q5, timestamp: new Date() });
        existingApp.lastApplicationTime = new Date();
        existingApp.lastStatus = 'Pending';
        await existingApp.save();
      } else {
        const newApp = new Application({
          userId: interaction.user.id,
          applications: [{ q1, q2, q3, q4, q5, timestamp: new Date() }],
      ...
      // Increment total applications stat
      await updateStats(interaction.guild.id, 'totalApplications');

      // Notify user
      await interaction.reply({ content: 'تم استلام طلبك بنجاح! شكراً لتقديمك.', ephemeral: true });

      // Send application details to staff room
      const serverSettings = await ServerSettings.findOne({ guildId: interaction.guild.id });
      if (!serverSettings || !serverSettings.staffroom) return;

      const staffRoom = interaction.guild.channels.cache.get(serverSettings.staffroom);
      if (!staffRoom) return;

      const appEmbed = new EmbedBuilder()
        .setTitle(`طلب تقديم جديد من ${interaction.user.tag}`)
        .setColor(config.embedcolor)
        .addFields(
          { name: 'الاسم الكامل', value: q1 },
          { name: 'العمر', value: q2 },
          { name: 'الخبرات', value: q3 },
          { name: 'سبب الانضمام', value: q4 },
          { name: 'أوقات التواجد', value: q5 }
        )
        .setTimestamp();

      await staffRoom.send({ embeds: [appEmbed] });
      await logSystem.sendLog(interaction.guild, `تلقى طلب تقديم جديد من ${interaction.user.tag}`, '#00ff00');

    } catch (error) {
      console.error('Error handling application submission:', error);
      await interaction.reply({ content: 'حدث خطأ أثناء تقديم الطلب. حاول مرة أخرى لاحقًا.', ephemeral: true });
    }
  }
}
});
